...
def list_all_books_in_library(library):
    """
    List all books in a specific library.
    Assumes a model 'Library' and a ForeignKey from 'Book' to 'Library'.
    """
    print(f"\n--- Query: Books in Library '{library}' ---")
    try:
        # 1. Find the specific Library instance
        books = Library.objects.get(name=library), books.all()

        print(f"Found {books.count()} books in {library}:")
        for book in books:
